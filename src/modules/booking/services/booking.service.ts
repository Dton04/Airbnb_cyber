...
export class BookingService {
  ...
  async create(dto: CreateBookingDto, reqUser: any) {
    ...
    if (soLuongKhach > phong.khach!) {
      throw new BadRequestException(
        `Phòng này chỉ chứa tối đa ${phong.khach} người`,
      );
    }
    ...
    if (user?.email) {
      setImmediate(() => {
        this.emailQueue
          ...
          .catch((err) =>
            console.error('[EMAIL] Failed to enqueue email job:', err?.message),
          );
      });
    }
  ...
  async update(id: number, dto: UpdateBookingDto, reqUser: any) {
    ...
    const ngayDen: Date = dto.ngayDen ? new Date(dto.ngayDen) : booking.ngay_den!;

    const ngayDi: Date = dto.ngayDi ? new Date(dto.ngayDi) : booking.ngay_di!;

    const soLuongKhach = dto.soLuongKhach ?? booking.so_luong_khach;
    ...
    if (soLuongKhach! > room.khach!) {
      throw new BadRequestException(`Phòng chỉ cho tối đa ${room.khach} khách`);
    }
