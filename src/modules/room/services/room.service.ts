...
export class RoomService {
  ...
  async create(dto: CreateRoomDto) {
    if (dto.maViTri !== undefined) {
      const viTri = await this.prisma.viTri.findFirst({
        where: { id: dto.maViTri, isDeleted: false },
      });
  ...
  async uploadImage(id: number, file: Express.Multer.File) {
    ...
    if (room.hinh_anh) {
      try {
        const urlParts = room.hinh_anh.split('/');
        const fileWithExtension = urlParts[urlParts.length - 1];
        const fileName = fileWithExtension.split('.')[0];
        const publicId = `airbnb/rooms/${fileName}`;
        await this.storageService.deleteFile(publicId);
      } catch (err) {
        console.error('Không thể xóa hình phòng cũ:', err);
      }
    }
